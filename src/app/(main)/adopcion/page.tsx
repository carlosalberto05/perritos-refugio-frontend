...
export default function AdoptionPage() {
  ...
  const filteredDogs = useMemo<Dog[]>(() => {
    ...
    return result;
  }, [dogs, searchQuery, selectedSize, selectedColor, sortBy]);

  if (!dogs) {
    return <div className="min-h-screen pt-24">Cargando...</div>;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl">
            Explora nuestra galería de perritos buscando un hogar. Los primeros
            que verás son los que están más cerca de ti.
          </p>
        </div>
              ...
              <div key={dog.id} className="relative group">
                {sortBy === 'distance' && dog.distance && (
                  <div className="absolute top-4 left-4 z-10 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-lg group-hover:scale-110 transition-transform">
                    <MapPin className="w-3 h-3" />A {dog.distance} km
                  </div>
                )}
                <DogCard
                  name={dog.name}
